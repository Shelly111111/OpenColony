...
import { ClaudeSDKClient, createSDKClient } from "./backends/sdk-client";
...
export class ClaudeUnifiedPtyManager {
  ...
  async runAll(commands: string[]): Promise<void> {
    ...
    // SDK模式：spawnClaude已同步等待完成，无需轮询marker文件
    // PTY模式：需要轮询completion marker文件检测完成
    if (this.mode === 'pty') {
